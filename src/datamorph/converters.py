...
class AvroWriter(FormatWriter):
    def write_stream(self, rows: RowStream, path: str | Path) -> int:
        ...
        for s_field in schema_data:
            avro_type = _avro_type_for_schema(s_field["type"])
            if s_field["name"] in nullable_fields and avro_type != "null":
                avro_type = ["null", avro_type]
            fields.append({"name": s_field["name"], "type": avro_type})
